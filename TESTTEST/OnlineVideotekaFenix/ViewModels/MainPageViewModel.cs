...
namespace OnlineVideotekaFenix.ViewModels
{
    public class MainPageViewModel :INotifyPropertyChanged
    {
        ...
        private ObservableCollection<Film> listaPretragaFilmova;
        private string azuriranjeFilmovaPretragaFilma;      
        public ObservableCollection<Film> ListaPretragaFilmova
        {
            get
            {
                return listaPretragaFilmova;
            }
            set
            {
                listaPretragaFilmova = value;
                NotifyPropertyChanged("listaPretragaFilmova");
            }
        }
        public string AzuriranjeFilmovaPretragaFilma
        {
            get { return azuriranjeFilmovaPretragaFilma; }
            set
            {
                azuriranjeFilmovaPretragaFilma = value;
                promijeniListu();
                NotifyPropertyChanged("AzuriranjeFilmovaPretraga");
            }
        }

        public void promijeniListu()
        {
            if (AzuriranjeFilmovaPretragaFilma.Count() == 0)
                foreach (Film f in videoteka.ListaFilmova)
                    ListaPretragaFilmova.Add(f);
            else
            {
                ListaPretragaFilmova.Clear();
                foreach (Film f in ListaPretragaFilmova)
                {
                   
                    if (f.NazivFilma.ToLower().Contains(AzuriranjeFilmovaPretragaFilma.ToLower()))
                    {
                        ListaPretragaFilmova.Add(f);
                    }
                }
        ...
        public async void InicijalizirajFilmove()
        {
            ...
            ListaPretragaFilmova = new ObservableCollection<Film>();
            foreach (FilmDB film in films)
            {
                videoteka.ListaFilmova.Add(new Film(film));
                ListaPretragaFilmova.Add(new Film(film));
            }
